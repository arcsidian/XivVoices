...
namespace XivVoices.Engine
{
    ...
    public class Engine
    {
        ...
        public Engine(Configuration _configuration, Database _database, Audio _audio)
        {
            if (Instance == null)
            {
                Instance = this;
            ...
            if (ffxivMessages.Count > 0)
            {
                if (ffxivMessages.TryDequeue(out XivMessage msg))
                {
                    if (msg.Network == "Online")
                    {
                        ...
                        //else if (Configuration.LocalTTSEnabled && !Configuration.WebsocketRedirectionEnabled && (msg.Reported || msg.Ignored)) // !&& AudioIsMuted?
        ...
        {
            this.Database.Plugin.Chat.Print("starting local tts");
            string pattern = "\\b" + "_NAME_" + "\\b";
            msg.Sentence = Regex.Replace(msg.Sentence, pattern, msg.TtsData.User.Split(' ')[0]);
            msg.OriginalSentence = msg.Sentence;

            if (localTTSMale == null)
                localTTSMale = TTSVoiceNative.LoadVoiceFromResources(speakerAI);
            var aitask = Audio.Engine.Speak(msg.OriginalSentence, localTTSMale);
            yield return new WaitUntil(() => aitask.IsCompleted);
            if (aitask.Exception != null)
            {
                Debug.LogError($"Error processing audio file: {aitask.Exception}");
                yield break;
            }
            AudioClip aiclip = aitask.Result;
            Audio.PlayAudioFromClip(msg, aiclip, messagesContainer, true);
        }
        */
            ...
            if (msg.FilePath.EndsWith(".ogg"))
            {
                ...
                try
                {
                    ...
                    if (changeSpeed || applyEffects)
                        waveStream = await FFmpegAndDecodeOggOpusToPCMAsync(msg);
                    else
        ...
        public static async Task<WaveStream> FFmpegAndDecodeOggOpusToPCMAsync(XivMessage msg)
        {
            changeTimeBusy = true;
            string outputFilePath = System.IO.Path.Combine(Engine.Instance.Database.DirectoryPath, "current" + Engine.Instance.Database.GenerateRandomSuffix() + ".ogg");
        ...
        static string SoundEffects(XivMessage msg, bool polly = false)
        {
            ...
            if (Engine.Instance.Configuration.Speed != 100) changeSpeed = true;
            ...
            if (changeSpeed)
            {
                ...
                filterArgs += $"\"atempo={(Engine.Instance.Configuration.Speed/100f).ToString(CultureInfo.InvariantCulture)}\"";
            }
